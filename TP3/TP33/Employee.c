...
int employee_buscarMayorId(LinkedList* pArrayListEmployee, int* id)
{
	...
	if(pArrayListEmployee!=NULL)
	{
		...
		*id = mayor + 1; // valor a asignar al proximo empleado
		retorno = 1;
	}
	return retorno;
}
	...
	if(pArrayListEmployee != NULL && id > 0 && indice != NULL)
	{
		*indice = -1;
		for(int i = 0; i < ll_len(pArrayListEmployee) ; i++)
		{
			...
			if(employee != NULL)
			{
				// se busca que el numero de id pasado como parametro coincida con el campo id de un empleado
				if(employee->id == id)
				{
					*indice = i;
					retorno = 1;
...
Employee* employee_newParametros(char* idStr,char* nombreStr,char* horasTrabajadasStr,char* sueldoStr)
{
	...
	if(newEmployee != NULL && idStr != NULL && nombreStr != NULL && horasTrabajadasStr != NULL && sueldoStr != NULL)
	{
		...
		{
			// si falla al menos uno de los setters se elimina el empleado y se carga el puntero con NULL
